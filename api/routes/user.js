...
router.get('/notification/:id', (req, res) => {
	User.find({ _id: ObjectId(req.params.id) }, { notification: true }).then((result) => {
		return res.status(200).json(result[0].notification);
	});
});

router.patch('/notification/reset/:userId', (req, res) => {
	User.updateOne({ _id: ObjectId(req.params.userId) }, { $set: { notification: [] } }).then().catch();
});
...
//////////////////////////////////////////////
router.post('/signup/:someId', uploadSignUp.single('profileImage'), (req, res, next) => {
...
router.patch('/follow/unFollow/:userId/:followersId', checkAuth, (req, res) => {
	...
	User.find({ _id: userId }).then((result2) => {
		if (result2.length !== 0) {
			...
			let alreadyFollows = followersArray.filter((item) => item.userId === followersId);
			let notFollowing = followersArray.filter((item) => item.userId !== followersId);
			if (alreadyFollows.length === 1) {
				User.updateOne({ _id: followersId }, { $pull: { following: { userId: userId } } })
						...
						transporter.sendMail(
							{
								...
								// text: `Hi ${req.body.fName}, the statement which you have uploaded on problemspotter is approved.
								//       The supporters like you is holding the civil field in technology era problemspotter.com/account/authentication/${userId}/${emailKey}`,
								html    : `<h1>Hi ${result2[0].fName +
									'  ' +
									result2[0].lName +
									','}</h1><br/><p>Dear user of problemspotter.com , <a href="https://civil.problemspotter.com/user/details/${req.body.name
									.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '')
									.replace(/ /g, '-')}/${req.body.userId}" > ${req.body
									.name}</a> has followed you. </p><img src='https://my-server-problemspotter.herokuapp.com/websiteLogo/newlogo.jpg' /><p> Checkout your profile on <a href="https://problemspotter.com/user/details/${result2[0]
									._id}" >here</a>  </p><p>Love from problemspotter.com ❤</p>`
							},
							function(error, info) {
								if (error) {
									console.log(error);
	...
	User.find({ email: req.body.email })
		.then((result) => {
			if (result.length === 0) {
				...
				let forgetKey =
					new mongoose.Types.ObjectId() + '_' + new mongoose.Types.ObjectId() + '_' + Math.random(0, 10000);
				User.updateOne({ email: req.body.email }, { $set: { forgetKey: forgetKey } })
					.then((result) => {})
					.catch((err) => {
						console.log(err);
					});
					...
					transporter.sendMail(
						{
							...
							// text: `Hi ${req.body.fName}, the statement which you have uploaded on problemspotter is approved.
							//       The supporters like you is holding the civil field in technology era problemspotter.com/account/authentication/${userId}/${emailKey}`,
							html    : `<h1>Hi ${response.fName +
								'  ' +
								response.lName}</h1><br/><p>Dear user of problemspotter.com, your email verification on problemspotter.com is successfully completed. <strong>You can log into your account now.</strong> </p><img src='https://my-server-problemspotter.herokuapp.com/websiteLogo/newlogo.jpg' /><p>Click on this link to log into you account <strong><a href='https://civil.problemspotter.com/login'  >click here</a></strong></p><p>Love from problemspotter.com ❤</p>`
						},
						function(error, info) {
							if (error) {
								console.log(error);
...
router.get('/all', (req, res) => {
	User.find({}, { lName: true, fName: true, _id: true, profileImage: true, email: true })
		.then(async (result) => {
			const newArray = [];
			for (let i = 0; i < result.length; i++) {
				newArray.push({
					text   : result[i].fName.toUpperCase() + ' ' + result[i].lName.toUpperCase(),
					name   : result[i].fName + ' ' + result[i].lName + i,
					value  : result[i].fName + ' ' + result[i].lName,
					url    : `https://civil.problemspotter.com/user/details/${result[i].fName +
						'-' +
						result[i].lName}/${result[i]._id}`,
					avatar : `https://my-server-problemspotter.herokuapp.com/image/profile/${result[i]._id}`,

					userId : result[i]._id,
					link   : `https://civil.problemspotter.com/user/details/${result[i].fName +
						'-' +
						result[i].lName}/${result[i]._id}`,
...
router.patch('/verify/authenticator/:id/:emailKey', (req, res) => {
	User.update({ _id: ObjectId(req.params.id) }, { $set: { emailKey: 'Verified', emailVerified: true } })
		.then((result) => {
			res.status(200).json({
...
router.patch('/statement/save/:userId', (req, res, next) => {
	...
	User.findOne({ _id: id })
		.then((data) => {
			arrayCheck = data.savedStatements.filter((item) => item._id === req.body._id);
...
router.patch('/statement/remove/:userId', (req, res, next) => {
	const id = req.params.userId;
	User.updateOne({ _id: id }, { $pull: { savedStatements: { _id: req.body._id } } })
		.then((result) => {
			res.status(200).json({
...
router.patch('/update/rating/:id', (req, res) => {
	User.findOne({ _id: ObjectId(req.params.id) })
		.then((result) => {
			...
			const checkUserId = userRatingArray.filter((item) => item.userId === req.body.userId);
						...
						User.updateOne(
							{ _id: req.body.userId },
							{
								$push : {
									activity : {
										action : 'rating',
										link   : `https://civil.problemspotter.com/user/details/${result.fName +
											'-' +
											result.lName}//${req.params.id}`,
										date   : new Date(),
										day    : new Date().getDay()
									}
								}
...
router.post('/login', (req, res, next) => {
	...
	User.find({ email: req.body.email })
		.then((user) => {
			/// Checking the user if have completed the waiting time for temporarily block //
			/////////////////////////////////////////////////////////////////////////
			if (user.length == 1) {
				try {
					blockedTime = user[0].blockTime ? user[0].blockTime : 0;
				} catch (error) {
					blockedTime = 0;
				}
				...
				if (current_time - blockedTime >= 10000 && user[0].ban.action) {
					User.updateOne(
						...
						.then((res) => {
							console.log('un ban success');
						})
						.catch((er) => {
							console.log('Some error occured');
						});
					user[0].ban.action = false;
				}
			}
			/////////////////////////////////////////////////////////////////////////
			try {
				attempts = user[0].incorrectAttempts ? user[0].incorrectAttempts : 0;
			} catch (error) {
				attempts = 0;
			}
			...
			bcrypt.compare(req.body.password, user[0].password, (err, result) => {
				...
				if (result) {
					...
					if (user[0].logInDetails.length >= 21) {
						loginDetails = user[0].logInDetails.slice(1, user[0].logInDetails.length);
					}
					...
					User.updateOne(
						{ _id: user[0]._id },
						{
							...
							$push        : {
								activity : {
									action : 'login',
									link   : `https://civil.problemspotter.com/user/details/${user[0].fName +
										'-' +
										user[0].lName}/${user[0]._id}`,
									date   : new Date(),
									day    : new Date().getDay()
								}
							}
				...
				attempts = attempts + 1;
				if (attempts < 6) {
					User.updateOne({ _id: user[0]._id }, { incorrectAttempts: attempts++ })
						.then((result4) => {})
						.catch((error) => {
							console.log(error);
						});
...
router.patch('/number/verification', checkAuth, (req, res) => {
	...
	User.updateOne({ _id: ObjectId(id) }, { $set: { numberVerified: true, number: number } }, { upsert: true })
		.then((result) => {
			return res.status(200).json({
